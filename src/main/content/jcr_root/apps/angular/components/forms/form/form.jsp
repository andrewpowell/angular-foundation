...
<form <c:if test="${hasController}">ng-controller="<c:out value="${controller}"/>" </c:if><c:if test="${hasNgSubmit}">ng-submit="<c:out value="${ngSubmit}"/>" </c:if><c:if test="${hasNgClick}">ng-click="<c:out value="${ngClick}"/>" </c:if>>
